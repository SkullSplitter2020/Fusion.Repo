...
MEDIA_ICON_PATH = os.path.join(ADDON_DIR, "resources", "media")
...
NIGHTMODE_FILE = os.path.join(ADDON_DIR, NIGHTMODE_FLAG)
HISTORY_LOG = os.path.join(ADDON_DIR, HISTORY_LOG_FILE)
AUTO_BACKUP_MARK = os.path.join(ADDON_DIR, AUTO_BACKUP_MARK_FILE)

# --- Multiuser ---
USER_FILE = os.path.join(ADDON_DIR, USER_FILE_NAME)
USER_CONFIG_JSON = os.path.join(ADDON_DIR, USER_CONFIG_JSON_FILE)


def SYNC_LOG(user=None):
    return os.path.join(ADDON_DIR, f"sync_protocol_{user or get_current_user()}.log")
...
def get_current_user():
    _log("Abrufen des aktuellen Benutzers.", xbmc.LOGDEBUG)
    if not os.path.exists(USER_FILE):
        _log(
            f"Benutzerdatei nicht gefunden: {USER_FILE}. Verwende 'default'.",
            xbmc.LOGINFO,
        )
        return "default"
    with open(USER_FILE, "r", encoding="utf-8") as f:
        user = f.read().strip() or "default"
        _log(f"Aktueller Benutzer: {user}.", xbmc.LOGDEBUG)
        return user


def set_current_user(name):
    _log(f"Setze aktuellen Benutzer auf: {name}.", xbmc.LOGINFO)
    with open(USER_FILE, "w", encoding="utf-8") as f:
        f.write(name.strip())
...
def get_user_config(user=None):
    ...
    _log(f"Lade Benutzerkonfiguration für: {user}.", xbmc.LOGDEBUG)
    cfg = load_json(USER_CONFIG_JSON, {})
    return cfg.get(user, {"sync_hours": 24})


def set_user_config(user, key, val):
    _log(f"Setze Benutzerkonfiguration für {user}: {key}={val}.", xbmc.LOGINFO)
    cfg = load_json(USER_CONFIG_JSON, {})
    if user not in cfg:
        cfg[user] = {}
    cfg[user][key] = val
    save_json(USER_CONFIG_JSON, cfg)


def FAVORITES_JSON_PATH():
    path = os.path.join(ADDON_DIR, f"favorites.{get_current_user()}.json")
    _log(f"Favoriten JSON Pfad: {path}.", xbmc.LOGDEBUG)
    return path


def HISTORY_JSON_PATH():
    path = os.path.join(ADDON_DIR, f"history.{get_current_user()}.json")
    _log(f"History JSON Pfad: {path}.", xbmc.LOGDEBUG)
    return path


def PLAYCOUNT_JSON_PATH():
    path = os.path.join(ADDON_DIR, f"playcounts.{get_current_user()}.json")
    _log(f"Playcount JSON Pfad: {path}.", xbmc.LOGDEBUG)
    return path
...
def load_theme():
    theme = "night" if os.path.exists(NIGHTMODE_FILE) else "default"
    _log(f"Geladenes Theme: {theme}.", xbmc.LOGDEBUG)
    return theme


def apply_color(label, color, night=None):
    if night is None:
        night = load_theme() == "night"
    if color == "badge":
...
def check_url_online(url):
    ...
    try:
        ...
        _log(
            f"URL {url} Status: {resp.status_code}, Online: {is_online}", xbmc.LOGDEBUG
        )
        return is_online
    ...
    for k, lst in streams.items():
        for s in lst:
            ...
            if not is_valid_url(url):
                label = f"[{k}] {s.get('name', '')}: [COLOR red]UNGÜLTIGE URL[/COLOR]"
                _log(
                    f"Ungültige URL gefunden: {url} in {k} / {s.get('name')}",
                    xbmc.LOGWARNING,
                )
            else:
                up = check_url_online(url)
                label = f"[{k}] {s.get('name', '')}"
                if up:
                    label += " [OK]"
                else:
                    label += " [COLOR red]OFFLINE[/COLOR]"
                    _log(f"Stream {s.get('name')} ({url}) ist OFFLINE.", xbmc.LOGINFO)
...
def play_stream(domain, name, url):
    ...
    # Add playcounts and history tracking
    playcounts = load_json(PLAYCOUNT_JSON_PATH(), {})
    playcounts[url] = playcounts.get(url, 0) + 1
    save_json(PLAYCOUNT_JSON_PATH(), playcounts)
    _log(f"Playcount für {name} auf {playcounts[url]} erhöht.", xbmc.LOGDEBUG)

    # Add to history
    history = load_json(HISTORY_JSON_PATH(), [])
    # Remove older entries for same URL to avoid duplicates, keep only latest
    ...
    _log(f"Stream {name} zur History hinzugefügt.", xbmc.LOGDEBUG)
...
def user_select():
    _log("Starte Benutzer-Auswahl.", xbmc.LOGINFO)
    users = ["default"]
    for fname in os.listdir(ADDON_DIR):
        if fname.startswith("favorites.") and fname.endswith(".json"):
            uname = fname.split(".")[1]
            if uname.endswith(".json"):  # Remove .json if it sneaks in
                uname = uname[:-5]
            if uname:  # Ensure username is not empty after splitting
                users.append(uname)
    users = sorted(list(set(users)))  # Remove duplicates and sort
    _log(f"Gefundene Benutzer: {', '.join(users)}", xbmc.LOGDEBUG)

    sel = xbmcgui.Dialog().select(
        "Benutzer wählen oder [+ Neuer User]", users + ["[Neuen Benutzer anlegen]"]
    )
    if sel == -1:  # Abgebrochen
        _log("Benutzer-Auswahl abgebrochen.", xbmc.LOGINFO)
        return

    if sel == len(users):  # "[Neuen Benutzer anlegen]" wurde gewählt
        name = xbmcgui.Dialog().input("Neuer Benutzername", type=xbmcgui.INPUT_ALPHANUM)
        if not name or not name.strip():
            xbmcgui.Dialog().notification(
                "Abbruch", "Ungültiger Benutzername.", xbmcgui.NOTIFICATION_WARNING
            )
            _log("Neuer Benutzername ungültig oder leer.", xbmc.LOGWARNING)
            return
        if name.lower() in [u.lower() for u in users]:
            xbmcgui.Dialog().notification(
                "Fehler", "Benutzername existiert bereits.", xbmcgui.NOTIFICATION_ERROR
            )
            _log(f"Benutzername '{name}' existiert bereits.", xbmc.LOGWARNING)
            return

        set_current_user(name.strip())
        xbmcgui.Dialog().ok("User gewechselt", f"Benutzerprofil '{name}' aktiv!")
        _log(f"Neuen Benutzer '{name}' erstellt und aktiviert.", xbmc.LOGINFO)
    else:
        set_current_user(users[sel])
        xbmcgui.Dialog().ok("User gewechselt", f"Benutzerprofil '{users[sel]}' aktiv!")
        _log(f"Benutzerprofil '{users[sel]}' aktiviert.", xbmc.LOGINFO)
    # Refresh the main menu after user change
...
# --- Nightmode Switch ---
def nightmode_switch():
    _log("Schalte Night-Mode um.", xbmc.LOGINFO)
    if os.path.exists(NIGHTMODE_FILE):
        os.remove(NIGHTMODE_FILE)
        xbmcgui.Dialog().ok("Night-Mode", "Night-Mode [AUS]")
        _log("Night-Mode ausgeschaltet.", xbmc.LOGINFO)
    else:
        with open(NIGHTMODE_FILE, "w"):
            pass  # Create empty file
        xbmcgui.Dialog().ok("Night-Mode", "Night-Mode [AN]")
        _log("Night-Mode eingeschaltet.", xbmc.LOGINFO)
    # Reload main menu to apply theme change
    xbmcplugin.endOfDirectory(ADDON_HANDLE, updateListing=True, cacheToDisc=True)
...
def create_release_save(reason="Manuell"):
    ...
    rel_zip = f"VIP_SNAPSHOT_{now}_{get_current_user()}.zip"
    rel_path = os.path.join(ADDON_DIR, rel_zip)
    important_files = [
        FAVORITES_JSON_PATH(),
        HISTORY_JSON_PATH(),
        PLAYCOUNT_JSON_PATH(),
...
def backup_gallery():
    ...
    for f_name, tag, f_type in files:
        full_path = os.path.join(ADDON_DIR, f_name)
        display = f_name
        if tag:
            if tag == "VIP-Release":
        ...
        if f_type == "zip":
            ...
            context_menu.append(
                (
                    "Bild zuweisen",
                    f"RunPlugin({BASE_URL}?action=set_image&name={urllib.parse.quote_plus(entry['name'])}&domain={urllib.parse.quote_plus(domain)})",
                )
            )
            if tag == "VIP-Release":
...
def gui_undo_restore():
    ...
    baks = []
    for fname in sorted(
        os.listdir(ADDON_DIR),
        key=lambda f: os.path.getmtime(os.path.join(ADDON_DIR, f)),
        reverse=True,
    ):
    ...
    if not baks:
        xbmcgui.Dialog().ok(
            "Restore Undo", "Keine Backup-Dateien (.bak_*) im Addon-Ordner gefunden!"
        )
        xbmcplugin.endOfDirectory(ADDON_HANDLE)
    ...
    bak, orig = baks[idx]
    f_bak = os.path.join(ADDON_DIR, bak)
...
def zip_backup():
    ...
    # Only allow selection of local addon data files
    addon_files = []
    for f in os.listdir(ADDON_DIR):
        if (
            f.endswith(".json")
            or f.endswith(".log")
            or f in [NIGHTMODE_FLAG, USER_FILE_NAME, USER_CONFIG_JSON_FILE]
        ):
    ...
    now = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    default_zip_name = f"backup_{now}_{get_current_user()}.zip"
    ...
    # Ensure zipname is a full path within ADDON_DIR
    if not os.path.isabs(zipname):
        zipname = os.path.join(ADDON_DIR, zipname)
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            for i in sel_indices:
                ...
                if os.path.exists(target_path):
                    ...
                    backup_path = f"{target_path}{BACKUP_SUFFIX}{timestr}"
...
def update_streams_from_url():
    url = "https://raw.githubusercontent.com/DeinRepo/streams.json"
    try:
        ...
        if response.status_code == 200:
            ...
            xbmcgui.Dialog().notification("Erfolg", "Stream-Liste aktualisiert")

    except Exception as e:
        _log(f"Update Fehler: {e}", xbmc.LOGERROR)


def set_custom_image(stream_name, domain):
    image_path = xbmcgui.Dialog().browse(2, "Bild auswählen", "images", ".png|.jpg")
    if not image_path:
        return
    ...
    # Aktualisiere Stream-Eintrag
    streams = load_json(STREAMS_JSON, {})
    for stream in streams.get(domain, []):
        if stream["name"] == stream_name:
            stream["icon"] = (
                f"resources/images/streams/{domain}/{_normalize_filename(stream_name)}.png"
            )
            break
...
def show_enhanced_history():
    history = sorted(
        load_json(HISTORY_JSON_PATH(), []), key=lambda x: x["last_played"], reverse=True
    )

    for item in history:
        li = xbmcgui.ListItem(
            label=f"{item['name']} [COLOR gray]({item['domain']})[/COLOR]"
        )
        li.setInfo(
            "music",
            {
                "title": f"Abgespielt: {item['play_count']}x",
                "duration": f"{item['total_duration']//60} min",
            },
        )

        # Context-Menü für Sharing
        context_menu = [
            (
                "Teilen",
                f"RunPlugin({BASE_URL}?action=share_stream&url={urllib.parse.quote_plus(item['url'])})",
            )
        ]
        li.addContextMenuItems(context_menu)
        ...
        xbmcplugin.addDirectoryItem(
            handle=ADDON_HANDLE,
            url=f"{BASE_URL}?action=play_stream&domain=history&name={urllib.parse.quote_plus(item['name'])}&url={urllib.parse.quote_plus(item['url'])}",
            listitem=li,
            isFolder=False,
        )
...
def search_streams(search_term):
    ...
    for domain, stream_list in streams.items():
        for stream in stream_list:
            search_text = f"{stream['name']} {domain} {stream.get('genre','')}".lower()
...
def show_search_results(results):
    for domain, stream in results:
        li = xbmcgui.ListItem(label=f"[{domain}] {stream['name']}")
        li.setProperty("IsPlayable", "true")

        xbmcplugin.addDirectoryItem(
            handle=ADDON_HANDLE,
            url=f"{BASE_URL}?action=play_stream&domain={urllib.parse.quote_plus(domain)}"
            f"&name={urllib.parse.quote_plus(stream['name'])}"
            f"&url={urllib.parse.quote_plus(stream['url'])}",
            listitem=li,
            isFolder=False,
        )
...
def main_menu(streams):
    ...
    xbmcplugin.addDirectoryItem(
        ...
        listitem=xbmcgui.ListItem(
            label=f"[COLOR lime]Aktueller Benutzer: {get_current_user()}[/COLOR]"
        ),
        isFolder=False,
    )
    # In der main_menu() Funktion hinzufügen:
    xbmcplugin.addDirectoryItem(
        handle=ADDON_HANDLE,
        url=f"{BASE_URL}?action=show_enhanced_history",
        listitem=xbmcgui.ListItem(label="★ Erweiterte History"),
        isFolder=True,
    )
    ...
    xbmcplugin.addDirectoryItem(
        ...
        listitem=xbmcgui.ListItem(label="🔍 Suche"),
        isFolder=False,
    )
    # Fügen Sie dies zur main_menu() Funktion hinzu:
    xbmcplugin.addDirectoryItem(
        handle=ADDON_HANDLE,
        url=f"{BASE_URL}?action=show_filter_menu",
        listitem=xbmcgui.ListItem(label="Filter/Sortierung"),
        isFolder=True,
    )
    ...
    xbmcplugin.addDirectoryItem(
        ...
        listitem=xbmcgui.ListItem(label="★ Favoriten"),
        isFolder=True,
    )
    xbmcplugin.addDirectoryItem(
        handle=ADDON_HANDLE,
        url=f"{BASE_URL}?action=add_stream",
        listitem=xbmcgui.ListItem(label="Stream hinzufügen"),
        isFolder=False,
    )
    ...
    xbmcplugin.addDirectoryItem(
        ...
        listitem=xbmcgui.ListItem(label="Playlist importieren"),
        isFolder=False,
    )
    specials = [
        # Benutzer- und Nutzerverwaltung
        (
            "Benutzer wechseln",
            "user_select",
            os.path.join(MEDIA_ICON_PATH, "multiuser.png"),
        ),
        # Stream Features
        (
            "Stream Health-Check",
            "health_check",
            os.path.join(MEDIA_ICON_PATH, "health.png"),
        ),
        (
            "Fehlende Thumbs/Icons",
            "missing_icons",
            os.path.join(MEDIA_ICON_PATH, "imagewarn.png"),
        ),
        (
            "Suchvorschläge/Autocomplete",
            "search_menu",
            os.path.join(MEDIA_ICON_PATH, "search.png"),
        ),
        # Backup, Galerie, Release-Save (immer verfügbar)
        ("ZIP Backup", "zip_backup", os.path.join(MEDIA_ICON_PATH, "zip.png")),
        ("Backup-Galerie", "backup_gallery", os.path.join(MEDIA_ICON_PATH, "zip.png")),
        (
            "Backups nach Tag filtern",
            "backup_tag_filter",
            os.path.join(MEDIA_ICON_PATH, "filter.png"),
        ),
        (
            "VIP-Snapshot (jetzt sichern)",
            "create_release_save",  # Changed action name
            os.path.join(MEDIA_ICON_PATH, "star.png"),
        ),
        # ZIP, Undo, Diagnose/Restore/Fix für JSON/ZIP (immer verfügbar)
        (
            "Letzten Restore rückgängig machen (Undo)",
            "gui_undo_restore",
            os.path.join(MEDIA_ICON_PATH, "undo.png"),
        ),
        (
            "Diagnose/Restore kaputter JSONs",
        ...
        (
            "Diagnose/Restore kaputter ZIP-Backups",
            "broken_zip_diagnose",
            os.path.join(MEDIA_ICON_PATH, "zip.png"),
        ),
    ]
    ...
    specials.extend(
        [
            ...
            ("--- Statistik/UI-Komfort ---", None, None),  # Separator
            (
                "Night-Mode/Light-Switch",
                "nightmode_switch",
                os.path.join(MEDIA_ICON_PATH, "nightmode.png"),
            ),
            (
                "Playcount/Streamstatistik",
                "streamstats",
                os.path.join(MEDIA_ICON_PATH, "stats.png"),
            ),
            (
                "Zuletzt hinzugefügt/geändert",
                "recentchanged",
                os.path.join(MEDIA_ICON_PATH, "update.png"),
            ),
        ]
    )
    ...
    for label, action, icon in specials:
        ...
        li = xbmcgui.ListItem(
            label=f"[COLOR yellow]{label}[/COLOR]" if theme == "night" else label
        )
        if icon:
