...
def read_body_and_headers(url, post=None, headers=[], follow_redirects=False, timeout=None):
    ...
    # one of the two cookie handling modules
        ...
        if cookielib is not None:
            _log("read_body_and_headers opener using urllib (cookielib)")
            # if we use cookielib
            # then we get the HTTPCookieProcessor
            # and install the opener in urllib
            if not follow_redirects:
                opener = urllib.build_opener(urllib.HTTPHandler(debuglevel=http_debug_log_enabled),urllib.HTTPCookieProcessor(cj),NoRedirectHandler())
            else:
                opener = urllib.build_opener(urllib.HTTPHandler(debuglevel=http_debug_log_enabled),urllib.HTTPCookieProcessor(cj))
    ...
    # Si falla la repite sustituyendo caracteres especiales
        ...
        req = urllib.Request(url.replace(" ","%20"))
...
def add_item(action="", title="", plot="", url="", thumbnail="", fanart="", show="", episode="", extra="", page="", info_labels=None, isPlayable=False, folder=True):
    ...
    listitem.setArt({'thumb': thumbnail, 'fanart': fanart})  # Setting thumbnail and fanart images
...
def direct_play(url):
    ...
    try:
        xlistitem = xbmcgui.ListItem( title, icon="DefaultVideo.png", path=url)
    except:
        xlistitem = xbmcgui.ListItem( title, icon="DefaultVideo.png", )
