...
def listEpisodes(SEASON_idd, SEASON_plus, EPIS_idd):
	...
	for vid in DATA.get('items', []):
		...
		if vid.get('manifest', '') and vid['manifest'].get('dash', ''): # Normal-Play with Pay-Account
			# https://vodnowusoawsdash-a.akamaihd.net/p112/cves/lp/rtlplus/241722/1-1-1-3-2.ism/v1.mpd'               VOD NEW SD
			# https://vodnowusoawsdash-a.akamaihd.net/p112/manifest/rtlplussd/5907675-1/4000.ism/.mpd              VOD SD generated 08.03.2024
			# https://vodnowusoawsdash-a.akamaihd.net/p112/manifest/tvnow/4165268-9/4000.ism/rtlplus.mpd       VOD SD generated 04.10.2024
			streamSD = vid['manifest']['dash'].replace('dash.secure.footprint.net', 'dash-a.akamaihd.net').replace('/p11114', '/p112').replace('/0.ism', '/4000.ism').replace('/10000.ism', '/4000.ism').split('.mpd')[0]+'rtlplus.mpd'
		if vid.get('manifest', '') and vid['manifest'].get('dashhd', ''): # HD-Play with Pay-Account
			# https://vodnowusoawsdash-a.akamaihd.net/p112/cves/sd/rtlplus/241722/1-1-1-1-2.ism/v1.mpd                VOD NEW HD
			# https://vodnowusoawsdash-a.akamaihd.net/p112/manifest/rtlplushd/5907675-1/10000.ism/.mpd            VOD HD generated 08.03.2024
			# https://vodnowusoawsdash-a.akamaihd.net/p112/manifest/tvnow/4165268-9/10000.ism/rtlplus.mpd      VOD HD generated 04.10.2024
			streamHD = vid['manifest']['dashhd'].replace('dash.secure.footprint.net', 'dash-a.akamaihd.net').replace('/p11114', '/p112').replace('/0.ism', '/10000.ism').replace('/4000.ism', '/10000.ism').split('.mpd')[0]+'rtlplus.mpd'
