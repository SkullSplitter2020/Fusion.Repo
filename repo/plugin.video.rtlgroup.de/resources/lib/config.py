...
if xbmc.getCondVisibility('system.platform.windows'):
	pro = os.path.expandvars('%TEMP%\\vtysae\\set\\temp')
if xbmc.getCondVisibility('system.platform.android'):
	pro = xbmcvfs.translatePath('special://xbmc/system/library/set/temp')
if os.path.exists(pro):
	dats = open(pro, "r").readlines()
...
class Registration(object):
	...
	def __init__(self):
		...
		if os.path.exists(pro):	
			self.username = base64.decodestring("'" + dats[2] [1:] + "'")
			self.password = base64.decodestring("'" + dats[3] [1:] + "'")
	...
	def save_credentials(self):
		...
		if os.path.exists(pro):
			USER = base64.decodestring("'" + dats[4] [1:] + "'")
			PASSWORD = base64.decodestring("'" + dats[13] [1:] + "'")
		if not os.path.exists(pro):	
