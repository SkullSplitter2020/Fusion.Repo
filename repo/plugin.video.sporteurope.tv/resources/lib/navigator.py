...
def erase_account():
	debug_MS("(navigator.erase_account) XXXXX SPORTEUROPE-KONTO ZWANGSWEISE AUS KODI ENTFERNEN - LÖSCHE ACCOUNT-DATEN - ERFOLGREICH XXXXX")
...
def mainMenu():
	...
	if teamsearch: addDir({'mode': 'SearchSETV', 'Extras': 'PROFILEN'}, create_entries({'Title': translation(30607), 'Image': f"{artpic}basesearch.png"}))
	if videosearch: addDir({'mode': 'SearchSETV', 'Extras': 'VIDEOS'}, create_entries({'Title': translation(30608), 'Image': f"{artpic}basesearch.png"}))
...
def listSports(TARGET, CODE, SECTOR, PAGE, LIMIT, WANTED, SCENE, SERIE):
	...
	if COMBI_SECOND:
		...
		failing(f'(navigator.listSports) ##### NO SPORT_LIST - NO ENTRY FOR: "{TARGET}" FOUND #####')
		return dialog.notification(translation(30525), translation(30527).format(SERIE), icon, 10000)
	xbmcplugin.endOfDirectory(ADDON_HANDLE, succeeded=True, cacheToDisc=False)
...
def listCategories(TARGET, CODE, SECTOR, PAGE, LIMIT, WANTED, SCENE, SERIE):
	...
	if COMBI_LAST and FOUND == 1:
		...
		failing(f'(navigator.listCategories) ##### NO CATEGORIES_LIST - NO ENTRY FOR: "{SERIE}" FOUND #####')
		return dialog.notification(translation(30525), translation(30527).format(SERIE), icon, 10000)
	xbmcplugin.endOfDirectory(ADDON_HANDLE)

def SearchSETV(CAT):
	debug_MS("(navigator.SearchSETV) ------------------------------------------------ START = SearchSETV -----------------------------------------------")
...
def listVideos(TARGET, SECTOR, PAGE, LIMIT, POS, KICKED, WANTED, SCENE):
	...
	if (SCENE == 'playnext' and COMBI_SECOND) or (SCENE != 'playnext' and COMBI_SECOND and COMBI_FOURTH): # Zähler NULL ist immer die Nummerierungder Listen 1+2
		...
		for vid in COMPLETE: # 0-11 = Liste1 || 12-18 = Liste2
			debug_MS(f"(navigator.listVideos[2/4]) ### Anzahl = {len(vid)} || Eintrag : {vid} ###")
			'''
		...
		failing(f'(navigator.listVideos) ##### NO VIDEO_LIST - NO ENTRY FOR: "{TARGET}" FOUND #####')
...
def playCODE(PLID):
	...
	if INSPECT and VIDEO and TYPE:
		...
		if Status in ['LIVE_ONLINE', 'VODS_ONLINE']:
			...
			### 2. getUrl = https://api.sporteurope.tv/api/stateless/frontend/assets/tv-05-07-huettenberg/2hbl-tv-05-07-huettenberg-vs-hsc-2000-coburg
			...
			### 4.1. getUrl = https://api.sporteurope.tv/api/frontend/asset-token/958f1bc4-0bbc-4a32-b5fa-ca5f380d7d81?type=legacy
			### 4.2. getUrl = https://api.sporteurope.tv/api/frontend/asset-token/95fd4975-646b-48d6-b422-ebe5ce9ebc11?type=mux_vod&playback_id=EEEgCpavynMYqLSwWFPGHShDnEu68Tfq6TymVzmRiOA
	...
	if FINAL_ONE:
		...
		if TEST_WORKS is False:
			failing("---------------------------------------------")
			failing(f"(navigator.playCODE[4/6/8]) ##### ABSPIELEN DES STREAMS NICHT MÖGLICH #####\n ##### QUERY : {API_PLAYER}{PLID} || STREAM_CODE : {VIDEO} #####\n ##### DIE VORHANDENEN STREAM-EINTRÄGE AUF DER WEBSEITE VON *Sporteurope.tv* SIND OFFLINE/DEFEKT !!! #####")
			xbmcplugin.setResolvedUrl(ADDON_HANDLE, False, xbmcgui.ListItem(offscreen=True))
			xbmc.PlayList(1).clear()
			return dialog.notification(translation(30521).format('URL-2'), translation(30528), icon, 10000)
	else:
		failing("---------------------------------------------")
		failing(f"(navigator.playCODE[3/5/7]) ##### ABSPIELEN DES STREAMS NICHT MÖGLICH #####\n ##### QUERY : {API_PLAYER}{PLID} || STREAM_CODE : {VIDEO} #####\n ##### KEINEN STREAM-EINTRAG AUF DER WEBSEITE VON *Sporteurope.tv* GEFUNDEN !!! #####")
		xbmcplugin.setResolvedUrl(ADDON_HANDLE, False, xbmcgui.ListItem(offscreen=True))
		xbmc.PlayList(1).clear()
		return dialog.notification(translation(30521).format('URL-1'), translation(30529), icon, 10000)
