...
import load_channels, six.moves.socketserver, socket, six.moves.SimpleHTTPServer, string, cgi, time
from os import curdir, sep
from six.moves.BaseHTTPServer import BaseHTTPRequestHandler, HTTPServer
import xbmc, xbmcvfs, xbmcaddon, xbmcgui, xbmcplugin, config, re, threading
            ...
            if re.match('.*channels-([0-9])\\..*|.*channels\\..*\\?portal=([0-9])', self.path):
                ...
                if searchObj.group(1) != None:
                    ...
                    try:
                        ...
                        data = load_channels.orderChannels(list(data['channels'].values()))
                        for i in data:
                            name = i['name']
                            cmd = i['cmd']
                            tmp = i['tmp']
                            ...
                            parameters = six.moves.urllib.parse.urlencode({'channel': cmd, 'tmp': tmp, 'portal': numportal})
...
def startServer():
    ...
    if server_enable != 'true':
        ...
        try:
            server = six.moves.socketserver.TCPServer(('', port), MyHandler)
...
def serverOnline():
    ...
    try:
        url = six.moves.urllib.request.urlopen('http://localhost:' + str(port) + '/online')
        code = url.getcode()
        if code == 200:
            return True
    except Exception as e:
...
def stopServer():
    port = addon.getSetting('server_port')
    try:
        url = six.moves.urllib.request.urlopen('http://localhost:' + str(port) + '/stop')
        code = url.getcode()
    except Exception as e:
        return
