...
def py2_enc(d, encoding='utf-8'):
	if PY2:
		d = d.encode(encoding)
	return d

def py2_dec(d, encoding='utf-8'):
	if PY2:
		d = d.decode(encoding)
	return d
...
def setSerialNumber(serial):
    ...
    if serial == None:
        ...
        if serial['custom'] == False:
            sn = hashlib.md5(py3_enc(mac)).hexdigest().upper()[13:]
            device_id = hashlib.sha256(py3_enc(sn)).hexdigest().upper()
            device_id2 = hashlib.sha256(py3_enc(mac)).hexdigest().upper()
            signature = hashlib.sha256(py3_enc(sn) + py3_enc(mac)).hexdigest().upper()
...
def retrieveData(url, values ):
	...
	req = Request(url + load, py3_enc(data), headers);
	resp = urlopen(req).read();
	
	if not is_json(resp):
		req = Request(url + load + '?' + data, headers=headers);
		resp = urlopen(req).read();
...
def getGenres(portal_mac, url, serial, path):	
	...
	if os.path.exists(portalurl):
		#check last time
		with codecs.open(portalurl, 'r', encoding='utf8') as data_file: data = json.load(data_file);
	...
	with codecs.open(portalurl, 'w', encoding='utf8') as f: f.write(data);
...
def getAllChannels(portal_mac, url, serial, path):
	...
	handshake(url);
	genrep = "_".join(re.findall("[a-zA-Z0-9]+", url));
	genrep = path + '/' + genrep + '-genres';
	with codecs.open(genrep, 'r', encoding='utf8') as f:
		a = f.read()
	genres=json.loads(a)
	...
	results = info['js']['data'];
	with codecs.open('/sdcard/results.txt','w',encoding='utf8') as k:
		k.write(repr(results))
...
def getEPG(portal_mac, url, serial, path):	
	...
	with codecs.open(portalurl, 'w', encoding='utf-8') as f: f.write(doc.toxml(encoding='utf-8'));
